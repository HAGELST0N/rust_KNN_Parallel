...
fn correct_classification_k3() {
    ...
    let results = rayon::bench(&train, &test, 3);
...
fn correct_classification_k1() {
    let (train, test) = make_dataset();
    let results = rayon::bench(&train, &test, 1);
    assert_eq!(results.correct, 2);
    assert_eq!(results.accuracy(), 100.0);
}
...
fn agrees_with_sequential() {
    ...
    let ray = rayon::bench(&train, &test, 3);
...
fn thread_counts_agree() {
    let (train, test) = make_dataset();
    let r1 = rayon::bench_with_threads(&train, &test, 3, 1);
    let r2 = rayon::bench_with_threads(&train, &test, 3, 2);
    let r4 = rayon::bench_with_threads(&train, &test, 3, 4);
    let r8 = rayon::bench_with_threads(&train, &test, 3, 8);
