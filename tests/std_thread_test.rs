...
fn correct_classification_k3() {
    ...
    let results = std_thread::bench(&train, &test, 3, 4);
...
fn correct_classification_k1() {
    let (train, test) = make_dataset();
    let results = std_thread::bench(&train, &test, 1, 4);
    assert_eq!(results.correct, 2);
    assert_eq!(results.accuracy(), 100.0);
}
...
fn agrees_with_sequential() {
    let (train, test) = make_dataset();
    let seq = sequential::bench(&train, &test, 3);
    let thr = std_thread::bench(&train, &test, 3, 4);
...
fn thread_counts_agree() {
    let (train, test) = make_dataset();
    let r1 = std_thread::bench(&train, &test, 3, 1);
    let r2 = std_thread::bench(&train, &test, 3, 2);
    let r4 = std_thread::bench(&train, &test, 3, 4);
    let r8 = std_thread::bench(&train, &test, 3, 8);
